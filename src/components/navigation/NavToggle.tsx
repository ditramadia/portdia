...
function NavToggle(props: NavToggleProps) {
  ...
  return (
    <div className="relative w-fit mt-1 flex gap-1 glass-blur px-2 py-2 rounded-full">
      <div className={`absolute -top-1 left-9 w-8 h-1 bg-neutral-100 rounded-full transition-transform ease-in-out duration-500 ${activeState === 'about' ? 'translate-x-[105px]' : 'translate-x-0'}`}>
      ...
      <div 
        className={`w-24 bg-gradient-to-r ${activeState === 'work' ? '' : 'hover:from-glass/10 hover:to-transparent'} px-2 py-2 rounded-full flex items-center justify-center body-m text-light cursor-pointer`}
        onClick={() => handleNavigate('work')}
      >
        Work
      </div>
      <div 
        className={`w-24 bg-gradient-to-l ${activeState === 'about' ? '' : 'hover:from-glass/10 hover:to-transparent'} px-2 py-2 rounded-full flex items-center justify-center body-m text-light cursor-pointer`}
